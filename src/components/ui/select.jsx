  ...
  const SelectContent = React.forwardRef(({ className, children, position = "popper", ...props }, ref) => (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(
          "relative z-50 overflow-auto min-w-[8rem] rounded-md border border-slate-200 bg-white text-slate-950 shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50",
          position === "popper" &&
        ...
        position={position}
        {...props}>
        <SelectScrollUpButton className="absolute top-0 left-0" />
        <SelectPrimitive.Viewport
          className={cn("p-1", position === "popper" &&
            "h-auto w-full min-w-[var(--radix-select-trigger-width)] max-h-60 overflow-auto")}>
          {children}
        </SelectPrimitive.Viewport>
        <SelectScrollDownButton className="absolute bottom-0 left-0" />
      </SelectPrimitive.Content>
    </SelectPrimitive.Portal>
  ));
